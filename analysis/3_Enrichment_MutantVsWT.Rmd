...
Overall we find that using multiple approaches for ranked lists and combining the p-values gives **fewer** enriched gene sets.
We believe that this offers a more prudent approach which reduces the impact of any bias that exists under any individual approach.
This is the premise of [EGSEA](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC5747338/), and whilst we prefer our specific methodology, we believe that EGSEA raises some important points regarding implicit bias in each method.
...
```{r hmMetaUpset, fig.width=10, fig.cap = "*UpSet plot indicating distribution of DE genes within all significant HALLMARK gene sets. Gene sets were restricted to those with an FDR < 0.05 and at least one DE gene. The plot is truncated at the right for simplicity. Most gene sets seem relatively independent of each other.*"}
hmMeta %>% 
...
goMeta %>%
  ...
  dplyr::filter(
    FDR < 0.01, nDE >= 10, shortest_path > 3
  ) %>%
  mutate_at(vars(one_of(c("wilkinson_p", "FDR", "adjP"))), formatP) %>%
  mutate(gs_name = str_trunc(gs_name, 70)) %>%
  dplyr::select(`Gene Set` = gs_name, `Number DE` = nDE, `Set Size` = gs_size, `Wilkinson~p~` = wilkinson_p, `p~FDR~` = FDR, `p~bonf~` = adjP) %>%
  pander(
    caption = "Results from combining all above approaches for the GO Gene Sets. All terms are significant using an FDR < 0.01. Only Gene Sets with at least ten DE genes, and more than 3 steps back to the Ontology Root are shown.",
    justify = "lrrrrr"
  )
```
...
```{r goMetaUpset, fig.height=6, fig.width=10, fig.cap = "*UpSet plot indicating distribution of DE genes within significant terms from the GO gene sets. For this visualisation, GO terms were restricted to those with 27 or more DE genes, where this represented more than 5% of a gene set as being DE, along with an FDR < 0.05. The plot is also truncated at the right hand side for simplicity. The 20 largest GO terms are shown, with genes from smaller GO terms being collected together as 'Other'. A group of 30 genes is relatively unique to RNA Metabolic Process, with an additional 28 being uniquely attributed to the Mitochondrial Envelope. Following these are 25 genes, spread across two clusters of terms which largely represent Ribosomal activity. *"}
goMeta %>% 
    mutate(propDE = nDE / gs_size) %>% 
    dplyr::filter(
      FDR < 0.05,
      propDE > 0.05, 
      nDE > 26
...
```{r heatRnaMetab, fig.height=8, fig.width=8, fig.cap="*All DE genes associated with the GO term 'RNA Metabolic Process'.*"}
go %>%
  dplyr::filter(
...
```{r heatRibo, fig.height=7, fig.width=8, fig.cap="*All DE genes associated with the GO term 'Ribosome'.*"}
go %>%
  dplyr::filter(
    gs_name == "GO_RIBOSOME",
    gene_id %in% dplyr::filter(deTable, DE)$gene_id
